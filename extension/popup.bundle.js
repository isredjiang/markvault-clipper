(() => {
  ...
  // extension/popup.js
  var DEFAULT_FOLDER = "_webClipper";
  var DEFAULT_ASSETS_FOLDER = "_webClipper/assets";
  var defaultTemplate = {
    id: "default",
    name: "\u9ED8\u8BA4",
    vaultName: "",
  ...
  async function getState() {
    ...
    const templates = Array.isArray(raw.templates) && raw.templates.length ? raw.templates : [{
      ...defaultTemplate,
  ...
  async function persistCurrentTemplate(message = text.savedTemplate) {
    const next2 = readEditor();
    const sameName = state.templates.find((item) => item.name === next2.name);
    if (sameName) {
      const target = { ...next2, id: sameName.id };
      state.templates = state.templates.map((item) => item.id === sameName.id ? target : item);
      state.activeTemplateId = sameName.id;
    } else if (next2.id === activeTemplate().id && activeTemplate().name === next2.name) {
      state.templates = state.templates.map((item) => item.id === next2.id ? next2 : item);
      state.activeTemplateId = next2.id;
    } else {
      const created = { ...next2, id: crypto.randomUUID() };
  ...
  function localMarkdownPayload(existingPayload) {
    ...
    let content = payload.selection ? htmlToMarkdown(`<p>${escapeHtml(payload.selection).replace(/\n/g, "<br>")}</p>`, payload.url) : article.markdown;
  ...
  function extractReadableArticle(payload) {
    ...
    try {
      ...
      const parsed = new import_readability.Readability(doc, {
        keepClasses: false,
        charThreshold: 500
      }).parse();
  ...
  function renderLocalTemplate(data) {
    ...
    const excerpt = data.excerpt ? ["> ## Excerpt", ...wrapText(data.excerpt, 100).map((line) => `> ${line}`), ""] : [];
    const frontMatter = [
      "---",
  ...
  function localDateTimeWithZone(now = /* @__PURE__ */ new Date()) {
  ...
  function wrapText(value, width) {
    ...
    if (!text2) return [];
    const lines = [];
    for (let index = 0; index < text2.length; index += width) {
      lines.push(text2.slice(index, index + width));
    }
    return lines;
  }
  ...
  function cleanupMarkdown(markdown) {
    return decodeEntities(String(markdown || "")).replace(/\u00a0/g, " ").replace(/[ \t]+\n/g, "\n").replace(/\n[ \t]+/g, "\n").replace(/[ \t]{2,}/g, " ").replace(/\n{3,}/g, "\n\n").replace(/^\s+|\s+$/g, "");
  }
  ...
  function cleanAuthor(value) {
    return cleanText(value).split(/\r?\n/).map((part) => part.trim()).filter(Boolean)[0]?.replace(/\s+\d+\s*人浏览.*$/i, "").replace(/\s*[·•|｜].*$/i, "").replace(/\s+(发表于|发布于|更新于|浏览|阅读).*$/i, "").replace(/\s+\d{4}[-/年]\d{1,2}[-/月]\d{1,2}.*$/i, "").trim() || "";
  }
  ...
  function slugify(value) {
    return String(value || "untitled").normalize("NFKC").replace(/[\\/:*?"<>|#^[\]]+/g, " ").replace(/\s+/g, " ").trim().slice(0, 90) || "untitled";
  }
